...
For advanced users, the Blueprint can run your own actions when it detects a version mismatch. If these are left empty, nothing changes.
...
target:
  entity_id: script.nspanel_update_firmware
data:
  variables:
    panel: "{{ nspanel_name }}"
    tag: "v{{ blueprint_version }}"
```
